...
async function loadSource(img, aud) {
    try {
        ...
        const response = await fetch('resources/image.json'); // Endpoint to list image resources.
        const resources = await response.json();
        resources.forEach(async resource => {
            await img.getImg(resource.name, resource.src); // Preload images.
        });
    } catch (error) {
    ...
    try {
        ...
        const response = await fetch('resources/audio.json'); // Endpoint to list audio resources.
        const resources = await response.json();
        resources.forEach(resource => {
            aud.addAudio(resource.name, resource.src); // Preload audio files.
        });
    } catch (error) {
